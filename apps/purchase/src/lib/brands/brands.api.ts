...
import type { Brand, BrandCode } from "./types";
...
export const getBrand = cache(
  async (code: BrandCode): Promise<Brand | null> => {
    try {
      const { data } = unwrap(
        await api().GET("/brands/{brandCode}", {
          params: { path: { brandCode: code } },
          ...nextCache({
            revalidate: BRAND_TTL_SECONDS,
            tags: [`brand:${code}`],
          }),
        }),
      );
